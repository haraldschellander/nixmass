...
#' 
#' Setting the option \code{return_full=TRUE}, the complete matrix of layer-wise snow depth, swe and age 
#' values can be returned. It can be used to produce plots of the temporal density evolution.  
#'
...
#' Dates must be given in the format \code{YYYY-MM-DD}, no sub-daily resolution is allowed at the moment (see details).
...
#' snow depth and age are returned (see details). Defaults to FALSE.
...
#' res <- swe.delta.snow(hsdata, return_full = TRUE)
#' res$matrices
#' 
#' # how many layers does the snowpack have at most? 
#' length(grep("ly", colnames(res$matrices)))
#' 
#' # how many days old is the oldest (first) layer?
#' library(dplyr)
#' lyrs <- res$matrices |> filter(param == "age") |> dplyr::select(starts_with("ly"))
#' sapply(lyrs, max)
#' 
swe.delta.snow <- function(data, rho.max = 401.2588, rho.null = 81.19417, c.ov = 0.0005104722, 
    ...
    if (!inherits(data$date, "character"))
        stop("date column must be of class 'character'")
    
    if (any(is.na(as.POSIXlt(data$date, format = "%Y-%m-%d"))))
        stop("date format must be '%Y-%m-%d'")
    
    if (any(!is.na(as.Date(data$date, format = "%Y-%m-%d %H:%M"))))
        stop("date format must be '%Y-%m-%d'")
    ...
    hobs_diff <- diff(Hobs > 0)
    i_start_snowperiod <- which(hobs_diff == 1)
    i_end_snowperiod <- which(hobs_diff == -1)
    
    if (length(i_end_snowperiod) == 0) {
        i_end_snowperiod <- length(Hobs)    
    }
    
    if (length(i_start_snowperiod) != length(i_end_snowperiod)) {
        i_end_snowperiod[length(i_start_snowperiod)] <- length(Hobs)
    }
    
    diff_mat <- cbind(i_start_snowperiod, i_end_snowperiod)    
    t_idx <- apply(diff_mat, 1, function(x){x[1]:x[2]})        
    
    # no snowfall period at all
    if (length(t_idx) == 0) {
            t_idx <- 1:(length(Hobs) - 1)    
    }
    
    # only one snowfall period
    if (is.matrix(t_idx)) {
        t_idx <- list(c(t_idx))
    }
    
    res <- lapply(t_idx, function(ti){
        ly_l <- list()
        
        for (ii in seq_along(ti)) {
            t <- ti[ii]
          
            #if (verbose) msg(m, t, paste("day", t, ": "))
            ...
            if (verbose) msg(m, t, "\n")
            if (return_full) {
                # collect internal matrices for each day
                ly_mat <- rbind(h[,2], swe[,2], age[,2]) |> 
                    as_tibble(.name_repair = "unique_quiet") |> 
                    mutate(param = c("h", "swe", "age"),
                           date = data$date[t],
                           ti = t) |> 
                    dplyr::select(c(ti, date, param), everything())
                colnames(ly_mat) <- c("ti", "date", "param", paste0("ly", 1:ly))
                ly_l[[ii]] <- ly_mat
            }
        }
        if (return_full) {
            list(results = cbind(ti = ti, SWE = SWE[ti]), 
                 matrices = bind_rows(ly_l))
        } else {
            cbind(ti = ti, SWE = SWE[ti])        
        }
    ...
    if (return_full) {
        # results
        res_all <- do.call(rbind, do.call(rbind, res)[, 1]) |> 
            as_tibble(.name_repair = "unique_quiet") |> 
            mutate(across("ti", ~ as.integer(.x)))
        idx_tot <- tibble(ti = seq_along(Hobs))
        res_all <- right_join(res_all, idx_tot, by = "ti") |> 
            arrange(ti) |> 
            replace_na(list(SWE = 0))
        
        # matrices
        idx_tot_mat <- lapply(c("h", "swe", "age"), function(param) {
            idx_tot$param <- param
            idx_tot$date <- as.Date(data$date)
            idx_tot
        })
        idx_tot_mat <- do.call(rbind, idx_tot_mat)
        mat_all <- do.call(bind_rows, do.call(rbind, res)[, 2]) |> 
            as_tibble(.name_repair = "unique_quiet") |> 
            mutate(date = as.Date(date)) |> 
            right_join(idx_tot_mat, by = c("ti", "date", "param")) |> 
            arrange(ti) |> 
            mutate(
                across(everything(), ~replace_na(.x, 0))
            )
        
        ret <- list(results = tibble(date = as.Date(data$date),
                                     Hobs = Hobs,
                                     SWE = res_all$SWE),
                    matrices = mat_all)
        return(ret)
        ...
        res_mat <- right_join(res_all, idx_tot, by = "ti") |> 
            arrange(ti) |> 
            replace_na(list(SWE = 0))
        return(res_mat$SWE)        
    }
